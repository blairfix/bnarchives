...
=item eprint_status (set)

The current status of the eprint record. By default this must be one 
of: C<inbox>, C<buffer>, C<archive>, C<deleted>.
...
The directory, relative to the documents directory for this 
repository, which this eprints data is stored in. E.g. 
C<disk0/00/00/03/34> for record 334.  This cannot be cloned or imported.
...
=item status_changed (time)

The date/time this record was moved between statuses (see C<eprint_status>).
...
The C<hide> option may be used to force an eprint to not appear in search/views
but is not considered a stable option.

=item fileinfo (longtext)

Stores a serialisation of the files including paths associated with the record.

=item item_issues (compound, multiple)

Stores any issues with the metadata for this record, which were reorted by a user 
of generated by running C<bin/issues_audit>.  Contains the following sub-fields:
...
=item item_issues_count (int)

The current number of issues with the record.  This field is volatile.
=back

=item edit_lock (compound)

Stores information about any edit lock on the record to prevent other users from
editing it.  Contains the following sub-fields:
...
A virtual field which represents the list of documents which are part of
this record.
...
L<SWORD API|http://www.ukoln.ac.uk/repositories/digirep/index/SWORD_Project>. 
...
Create a new EPrint entry in the given C<$dataset>..

If C<$data> is defined, then this is used as the base for the new 
record. Otherwise the repository specific defaults provided by 
 L</set_eprint_defaults> are used.

The fields C<eprintid> and C<dir> will be overridden even if they are 
set.
...
=item $dataobj = EPrints::DataObj::EPrint->create_from_data( $session, $data, $dataset )
 
Create a new object of this type in the database. 

C<$datase>t is the dataset it will belong to. 
 
C<$data> is the data structured as with 
L<EPrints::DataObj#new_from_data>.
...
=item $fields = EPrints::DataObj::EPrint->get_system_field_info
...
sub render
{
	...
	else
	{
		...
		my $content = $self->{session}->make_element( "div", class=>$summary_content_classes->{ROOT} );
		my $content_top = $self->{session}->make_element( "div", class=>$summary_content_classes->{top} );
		my $content_left = $self->{session}->make_element( "div", class=>$summary_content_classes->{left} );
		my $content_main = $self->{session}->make_element( "div", class=>$summary_content_classes->{main} );
